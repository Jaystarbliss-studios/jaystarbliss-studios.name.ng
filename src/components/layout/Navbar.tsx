    ...
    >
      <div className="container mx-auto px-4 md:px-8 max-w-7xl">
        <div className="flex items-center justify-between">
          ...
          <button 
            className={`lg:hidden relative z-50 p-2 ${isScrolled || mobileMenuOpen ? 'text-brand-slate dark:text-white' : 'text-white'}`}
        ...
        <div className="flex flex-col h-full pt-24 px-6 pb-12 overflow-y-auto">
          ...
          <div className="mt-auto pt-8 flex flex-col gap-4">
            ...
            <Link to="/portal" onClick={() => setMobileMenuOpen(false)} className="relative inline-flex w-full overflow-hidden rounded-xl p-[1px] focus:outline-none shadow-lg shadow-brand-red/20 group">
              <span className="absolute inset-[-1000%] animate-[spin_3s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#B91C1C_0%,#F8FAFC_50%,#B91C1C_100%)]" />
              <span className="inline-flex h-full w-full items-center justify-center rounded-xl bg-brand-slate dark:bg-slate-900 px-6 py-4 text-center font-bold text-white backdrop-blur-3xl transition-colors group-hover:bg-brand-red/90">
                LOGIN / SIGNUP
              </span>
            </Link>
            <Link 
              to="/services" 
              onClick={() => setMobileMenuOpen(false)}
              className="bg-brand-neutral dark:bg-slate-800 text-brand-slate dark:text-white border border-gray-200 dark:border-slate-700 px-6 py-4 rounded-xl text-center font-bold"
            >
              HIRE US
            </Link>
          </div>
