...
// 1. Add SCHOOL_NAMES definition
if (!html.includes('const SCHOOL_NAMES =')) {
    html = html.replace('import {  getFirestore,', `const SCHOOL_NAMES = {
...
// 2. Fix item is not defined in loadNewsCorner
html = html.replace(/var badgeColor = item\.type === 'link'.*?secondary-container';/g, '');

// 3. Add studentResourcesGrid population (loadStudentResources) and handlers
...
html = html.replace('// ── End UI Setup ──', studentResourcesJS + '\n// ── End UI Setup ──');

fs.writeFileSync('jaystarbliss-studios.name.ng/htdocs/pages/dashboard/admin-dashboard.html', html);
console.log("Fixed missing items in admin-dashboard.html");
