...
const oldButton = `
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-b from-[#df4627] to-[#b3290e] text-white font-bold tracking-widest font-mono text-xs uppercase rounded-xl py-4 mt-6 shadow-[0_10px_20px_rgba(223,70,39,0.3)] hover:shadow-[0_15px_30px_rgba(223,70,39,0.4)] hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 border border-[#f05637] disabled:opacity-50 disabled:hover:translate-y-0"
              >
`.trim();
...
file = file.replace(/<\/button>\s*<\/form>/, "</span>\n              </button>\n            </form>");
