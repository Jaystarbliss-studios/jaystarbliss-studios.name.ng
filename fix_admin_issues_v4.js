...
async function loadStudentResources() {
  ...
  } catch(e) { grid.innerHTML = emptyMsg('Error loading student resources: ' + e.message); }
}
...
if (html.includes(studentResourcesJS)) {
    console.log("Already has it");
} else {
    // Just inject before </body>
    html = html.replace(/<\/body>\s*<\/html>/, studentResourcesJS + '\n</body>\n</html>');
    fs.writeFileSync('jaystarbliss-studios.name.ng/htdocs/pages/dashboard/admin-dashboard.html', html);
    console.log("Fixed missing items in admin-dashboard.html v4");
}
